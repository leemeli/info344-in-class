...
func main() {
	...
	//using a channel to pass toFetch to fetch
	...
	for result := range results {
		...
		if result.Error != nil {
			...
			continue // will jump back to top of for loop and process next result instead
		}
		log.Printf("processing %d links found in %s", len(result.PL.Links),
			result.URL)
